...
スライドを順に映しながら読み上げ、字幕を出し、時間どおりに次へ進む。中身は
`slides/<名前>.js` というテキストファイル 1つ。テンプレートとガイドと原稿を AI に
渡して書いてもらうか、19種類のテンプレートをコピーして差し替える。書けたら
`player.html` と 2つ並べて Web サーバーに置き、URL を渡すだけ
（見る側に要るのはブラウザだけ）。

## 他に無い点

- **AI に書かせられる。** 渡すのはテンプレートとガイドと原稿だけ。スライド 1式が
  テキストファイル 1つなので、AI が書いたものをそのまま保存すれば動く。直すときも
  「3枚目をもっと短く」と文章で頼める
- **依存関係が無い。** 見る側はブラウザだけ。作る側も、コピーして書き換えて
  ブラウザで開くところまでは何も入れずに進む
- **読み上げ・字幕・自動送りが最初から付いている。** 音声を録る作業が要らない。
  ナレーションの文章を書けば喋り、言い回しを直すのも文章の書き換えだけで済む
