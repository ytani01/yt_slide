...
再生にはビルドもインストールも不要。`player.html` と `slides/<名前>.js` を
...
**AI に書かせてもよい。** `slides/template.js` と
[docs/User.md](docs/User.md) を渡して、作りたい内容を伝えれば `.js` を
書いてくれる（このリポジトリのスライドもそうやって作った）。
[依頼文の例と保存・確認の手順](docs/User.md#ai-に作ってもらう)もガイドにある。
