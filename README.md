...
**スライドは JavaScript ファイル 1 つ。ただし 1 から書かなくていい。**

スライドを順に映しながら読み上げ、字幕を出し、時間どおりに次へ進む。
中身は `slides/<名前>.js` に書く。19 種類の型をコピーして差し替えるか、
型とガイドを AI に渡して書いてもらう。書けたら `player.html` と 2 つ並べて
Web サーバーに置き、URL を渡すだけ（見る側に要るのはブラウザだけ）。
...
[型や AI で作る](docs/User.md#2-最初の-1-枚を書いて再生する) →
...
1 枚は、題名・読み上げる文章・本文を並べただけのもの。次は 1 枚分の断片で、
...
**1 から書かなくてよい。** 表紙、箇条書き、2 カラム比較、表、図解、引用、
時系列、Q&A など、
**[19 種類の型](https://ytani01.github.io/yt_slide/player.html?slides=template)**
が `slides/template.js` に入っている。近い型をまるごとコピーして、
題名・読み上げる文章・本文を差し替えるのが早い。

**AI に書かせてもよい。** 渡すのは 3 つ — `slides/template.js`（型の見本）、
