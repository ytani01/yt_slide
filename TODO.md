...
**残っている項目: TODO-108。** これまでに 107 件を決着させた。
新しく足すときは「完了済み」の上に節を作る。**番号は `TODO-109` から。**
...
- [ ] バッジ「AI Presentation Video」を消す
- [ ] 説明文「インタラクティブ・プレゼンテーション動画プレイヤー」を消す
- [ ] スライドタイトルの下に `slides/<名前>.js` を出す（狭い画面でも表示）

ヘッダーの左には、アイコン・スライドタイトル・JS ファイル名だけあればよい。
バッジと説明文は場所を取るだけで、見る人には要らない。どのスライドを
開いているかは、タイトルよりファイル名のほうが確かめやすい。

対象は `player.html` の `<header>` と `startApp()`。分岐や条件式は変えないので
reviewer は付けず、確認だけ verifier に分ける（広い画面と狭い画面で、
ヘッダー左の表示とレイアウトを実測させる）。
