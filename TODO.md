...
**残っている項目: TODO-105。** これまでに 104 件を決着させた。
新しく足すときは「完了済み」の上に節を作る。**番号は `TODO-106` から。**
...
利用者から「スライドを作って公開したい人が README を見て気になるところ」を
尋ねられ、README と `docs/User.md` を確認した。次の 5 点が迷いやすい。

1. 「Web サーバーに置く」とあるが、公開先を持っていない人向けの具体的な手順が無い。
2. 冒頭の「インストール不要」と、作成時の Python・uv・CLI の準備との関係が見えにくい。
3. 「player.html を開く」前に、必要なファイルをどう入手するかがその場に無い。
4. 型と AI の案内はあるが、README のコードは断片で、保存して動かすまでを想像しにくい。
5. 相手の端末での音声の違いと、音が出ないときの対処への案内が README から見えにくい。

この指摘に対し、利用者から修正案を TODO 項目として作るよう依頼された。
今回は項目の登録まで。本文の修正にはまだ着手しない。
README は入口、詳しい手順は `docs/User.md` とする既存の文書方針を保つ。

### やること

- [ ] README に「用意する → 型や AI で作る → 手元で確認する → 公開先に置く → URL を渡す」
  という全体の流れを短く示し、それぞれの手順へ直接リンクする。
- [ ] 閲覧・作成・公開で必要な準備を区別する。CLI が必要な操作と、ファイルのコピー・編集だけで
  できる操作を実装と照合し、「インストール不要」の適用範囲を明確にする。
- [ ] ファイルの入手から最初の 1 枚の再生まで、推奨する手順を途切れずに辿れるようにする。
  README の断片例には位置付けを示し、保存して動く完全な例へ案内する。
- [ ] AI に渡す依頼文の例と、必要な資料・生成ファイルの保存先・確認方法を
  `docs/User.md` にまとめ、README から案内する。
- [ ] 公開先を持っていない人向けに、公開先の用意・ファイル配置・共有 URL の確認までの
  具体例を `docs/User.md` に追加する。既存の Web サーバーへ置く手順も残す。
- [ ] ネット接続の必要性、端末による声の違い、音が出ないときの対処を短く案内し、
  詳細は既存のガイドへリンクする。
- [ ] `slides/readme.js` と `slides/user.js` の対応する説明を照合し、修正によって
  食い違う箇所があれば同期する。プレイヤーや CLI の機能追加は含めない。

### 調査後に決めること

公開手順の具体例に採用するサービスは、着手後に公式資料で前提条件と操作を確認してから決める。
既存デモと同じ GitHub Pages を第一候補に、アカウントの準備、操作の量、公開範囲を調べる。
初心者向けの例として適さない制約が見つかった場合は、代案との違いを示し、本文を作る前に
利用者へ確認する。実際の外部公開はこの項目の作業に含めない。

### 検証方法

- [ ] reviewer が README・ガイド・対応するスライドと実装を照合し、上記 5 点への回答が
  見つかること、手順の抜けと説明の食い違いが無いことを確認する。
- [ ] reviewer の指摘を反映した後、verifier が空の作業ディレクトリから掲載手順を再現する。
  最小の例を保存し、ブラウザで `player.html?slides=sample` の本文と読み上げを確認する。
- [ ] 公開用ファイルだけを別ディレクトリに配置して HTTP 配信し、案内した URL で
  スライドと画像が読み込めることを確認する。公開サービス固有の操作は公式資料と照合し、
  実際の外部公開をしていない点を検証結果に明記する。
- [ ] 文書の相対リンクと見出しへのリンクを確認する。スライドを変更した場合は
  対象の `player.html?slides=<名前>` をブラウザで開いて表示と操作を確認する。

main が修正し、別担当が説明の整合性と手順の再現を順に確認する。
着手時に `archives/agents/TODO-105/` へ分担理由と依頼を置き、各担当の報告を残す。
実際に用いたモデル・reasoning effort は完了時に記録し、測定できない token・料金は推測しない。
