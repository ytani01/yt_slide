...
**残っている項目: TODO-089、TODO-090。** これまでに 88 件を決着させた。
...
- [ ] 一覧に要るデータ（説明文・アイコン）をどこに持たせるか利用者と決める
- [ ] 生成先を `index.html` 直書きにするか、別ファイルにするか決める
- [ ] `tools/make-index.py`（仮）を作る
- [ ] `docs/User.md` の「手順」から「`index.html` に `<li>` を足す」を差し替える
...
**方針を変えた（2026-09-21）。ビルド不要に固執しない。** 立てたときは
`README.md` 冒頭の「ビルドもインストールも不要」を理由に生成案を外し、
`player.html` の側で `slides/` を列挙できるかを 3 環境（`file://`・Apache・
GitHub Pages）で実測する、という組み立てだった。`duration` を合わせるのに
どのみち `tools/` のスクリプトを走らせるので、**そのついでに一覧も生成する**。
ブラウザ側での列挙は検討しないので、3 環境の実測は要らない。

yt_slide の主眼は次の 2 つで、ビルドの有無はそこに含まれない。

- 既存のスライド作成サイトなどに頼らない
- 好きなサーバーに必要なファイルを置けば、ナレーション付きスライドを公開できる

`measure-duration.py` は 1 つのスライドの `duration` を測る道具で、一覧生成は
`slides/*.js` を全部見る別の仕事。**同じスクリプトには混ぜず、別に置く。**

決めること 1: **一覧に出す説明文とアイコンをどこから取るか。** いまの
`index.html` は項目ごとに説明文（「このリポジトリの紹介」など）と
FontAwesome のアイコン名を持っているが、`slides/*.js` の `slidesConfig` には
`title` と `heading` しか無い。`slidesConfig` に足すか、一覧では出さないか。

決めること 2: **生成先。** `index.html` を直接書き換えるか、一覧のデータだけ
別ファイル（`slides/_list.js` のような）に出して `index.html` がそれを読むか。
後者なら `index.html` を手で直す必要が無く、差分も小さい。

`README.md` 冒頭の「ビルドもインストールも不要」をどう書き直すかは
**TODO-090 に含める**（作る側に道具が要る、という同じ話のため）。
