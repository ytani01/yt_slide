...
**残っている項目: TODO-112〜114。** これまでに 111 件を決着させた。
新しく足すときは「完了済み」の上に節を作る。**番号は `TODO-115` から。**
...
- [ ] `slides/<名前>.js` の書き間違いで何が起きるかを、実際に壊して確かめる
- [ ] 事前チェック（`ytslide check` など）と、再生時の画面への案内の、
      どちらをどこまでやるかを決める
- [ ] 決まったら、実装は別項目として立てる

スライドは JavaScript ファイルなので、カンマの抜け・括弧の閉じ忘れ・
必須キーの書き忘れで読み込みに失敗する。今は画面がどうなるか（真っ白か、
既定のスライドに戻るか、何も出ないか）を確かめていない。非プログラマが
書く前提（TODO-103）なら、失敗したときに「どのファイルの何行目が怪しいか」が
分かる必要がある。

**決めること** — (1) 事前チェックを `ytslide` 側に置くか、`player.html` が
読み込み時に見るか、両方か。(2) チェックする範囲（構文エラーだけか、
`title`・`narration`・`duration` の欠け、`duration` と実測の食い違い、
...
- [ ] README の冒頭で、次の 5 点が最初の数行で伝わるようにする
  - AI に書かせられる（テンプレートとガイドを渡すだけ）
  - 依存関係が無い（見る側も作る側もブラウザだけで済む）
  - 読み上げ・字幕・自動送りが最初から付いている
  - MP4（＋`.srt`）に書き出せる
  - スライドがテキストファイルなので、git・grep・sed がそのまま効く。
    **どこを直したかが差分で分かり、用語や製品名は全スライドまとめて直せる**
- [ ] **利点は「何ができる」ではなく「何が嬉しいか」まで書く。**
      道具の名前（git・grep・sed）だけ並べない
- [ ] テキストであることの利点は、`docs/User.md` 側に節を作って厚く書く
  - **版管理**。git で差分が読める、レビューできる、巻き戻せる
  - **一括置換**。用語・製品名・バージョン番号を全スライドまとめて直せる
  - **自動生成**。データやログからスライドを機械的に作れる
  - **自動化**。更新を検知して MP4 を書き出す、といった処理を組める
  - **形式に閉じ込められない**。平文なので、あとから他のツールへ移せる
  - **エディタを選ばない**。オフラインでも ssh 越しでも書ける
- [ ] `slides/readme.js` も同じ内容に合わせる
- [ ] **他のサービス・ツールを名指ししない。**「〜より」「〜と違って」も書かない
- [ ] **セキュリティ・プライバシーには言及しない。**
      自前のサーバーに置ける点や、データを預けない点は書かない

「できること」の箇条書きは機能の列挙になっていて、どれが他に無い点なのかが
読み手に分からない。上の 5 点は揃っているものが少ないので、機能の一覧とは
別に前へ出す。

README の冒頭は 1 行に留め、内訳は `docs/User.md` へ置く（冒頭に並べると
訴求が散る）。

**確認が要る理由** — 書いた主張が実装と合っているかを、書いた本人が疑えない
（TODO-088 と同じ）。verifier には、5 点それぞれが今の `player.html` と
`ytslide` で本当にできるかを確かめさせる。「自動生成」「自動化」は
例を 1 つ実際に動かして確かめる。
