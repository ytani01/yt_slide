...
**残っている項目: 1 件。** これまでに 108 件を決着させた。
新しく足すときは「完了済み」の上に節を作る。**番号は `TODO-110` から。**

---

## TODO-109. player のヘッダー左のアイコンからスライド一覧へ戻れるようにする

player.html を開くと、スライド一覧（`index.html`）へ戻る手段が画面に無い。
ブラウザの戻るしか無く、URL を直接開いたときは戻れない。

ヘッダー左のアイコン（`player.html` の `w-8 h-8` の `<div>`）を
`index.html` へのリンクにする。同じタブで遷移し、`title` は
「スライド一覧へ戻る」。タイトルとファイル名は当たり判定に入れない
（利用者と相談済み）。
...
- 表示だけの変更で分岐は増えないので reviewer は立てない
- 確認は verifier に分ける。player.html を開いてアイコンを押し、
  `index.html` に遷移することを実測させる
