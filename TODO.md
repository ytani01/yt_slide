...
- [ ] 付録の表の 30 件を `fontawesome.com/icons/<名前>?s=solid` へのリンクにする
- [ ] verifier に 30 件の URL が実際に開けるか確かめさせる

TODO-069 で付録の表を作ったが、**Markdown ではアイコンの絵が出ない**ので、
名前だけ見ても何のアイコンか分からない。名前を FontAwesome のページへの
リンクにすれば、1 クリックで実物を見られる。

- **表は残す**（`icon` に貼るコピー元として要る）。リンクにするだけ
- スライドへの案内（付録の最後の 1 行）もそのまま残す。**スライドへの
  リンクに置き換えない**。`docs/User.md` は GitHub やエディタでも読まれ、
  HTTP で配信していないとスライドは開けないため
- リポジトリに画像を持たない（一覧を足すたびの撮り直しを避ける）
- URL のスラグはクラス名から `fa-` を除いたもの。**思い込みで書かず、
  実際に開けるかを確かめる**（名前が違えば 404 になるので、
  間違いに気付く手がかりにもなる）
- `slides/user.js` は変えない（スライドではリンクを踏めない）
