...
**残っている項目: TODO-093。** これまでに 92 件を決着させた。
新しく足すときは「完了済み」の上に節を作る。**番号は `TODO-094` から。**

---

## TODO-093. `index.html` に `README.md` を載せる

- [ ] スライド一覧の上に `README.md` をレンダリングして表示する
- [ ] `<details open>` で包み、クリックで折り畳めるようにする
- [ ] `README.md` が無ければ（fetch が失敗すれば）何も出さず、今までどおりの見た目にする

`index.html` を置いたディレクトリに `README.md` があるなら、一覧だけでなく
その中身も入口で読めるようにする。

変換は marked.js（cdnjs）で実行時に行う。`index.html` に `<details>` と
スクリプト、marked の出力向けの最小限の CSS（見出し・箇条書き・コード・
リンク）を足す。ビルド時に埋め込む案（`tools/make-index.py` で変換）も
あったが、README を直すたびに実行が要るので採らない。
`tools/make-index.py` は触らない（書き換えるのはマーカーの間だけなので
影響しない）。

分担は、実装が main、確認が verifier（README あり・無し、開閉の両方を
ブラウザで実測）、分岐が増えるので reviewer も別に立てる。
