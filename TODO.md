...
**残っている項目: TODO-073・TODO-074・TODO-075・TODO-076・TODO-077・TODO-078。** これまでに 72 件を決着させた。
...
**背景:** 初めて見る人が操作を見つけやすくするための提案。
`player.html` には Space・左右矢印・F・M・Escape のキー操作と
タップ・スワイプがあるが、画面上で一覧を確認する操作ガイドがない。
アイコンだけのボタンもあり、機能に気づきにくい。

- [ ] プレイヤーから開閉できる操作ガイドを付け、キー操作とタッチ操作を示す
- [ ] アイコンだけの操作ボタンに、機能が分かる読み上げ用の名前と説明を付ける
- [ ] ガイドをキーボードで開閉でき、閉じたら元のボタンへフォーカスが戻るようにする
- [ ] `docs/User.md` に操作ガイドの開き方を書く

**検証方法:** verifier が Playwright で 1280×800 とタッチ設定の 390×844 を
確認する。ガイドの開閉、Tab 移動、フォーカスの復帰、説明と実際のキー・
タッチ操作の一致を確かめる。ガイド操作で意図せず再生・前後送りが起きないこと、
小さい画面でも説明と閉じるボタンが欠けないことを確認する。

**着手前の担当見込み:** main が実装と文書を担当し、reviewer がイベント処理と
操作の整合性を確認したあと、verifier が実測する。実装は未着手。
