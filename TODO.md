...
**残っている項目: TODO-087。** これまでに 86 件を決着させた。
新しく足すときは「完了済み」の上に節を作る。**番号は `TODO-088` から。**
...
- [ ] 公開 URL が開き、スライドが再生できることを確かめる

`develop` のルートを公開元にして GitHub Pages を有効にした
（https://ytani01.github.io/yt_slide/ ）。参照は全部相対パスで、ビルドも
要らないので設定だけで済むはずだった。

ところが Jekyll のビルドが `Page build failed.` で失敗する。ブランチ公開
（legacy）方式はビルドログが API からもブラウザからも取れず、原因を直接
確認できない。Liquid 構文（`{{` `{%`）との衝突は調べたが、引っかかったのは
画像 4 件だけで、中身がバイナリなので Jekyll の処理対象外だった。

`.nojekyll` を置くと Jekyll の処理を丸ごと飛ばして静的ファイルをそのまま
配信するので、原因を特定しなくてもこの手の失敗をまとめて回避できる。

完了条件は、https://ytani01.github.io/yt_slide/ が 200 を返し、一覧から
`player.html?slides=readme` が開いて再生まで進むこと。実機で確かめるので
verifier を別に立てる。ファイルを足すだけで分岐は変わらないため reviewer は
立てない。

手で蹴り直した 3 回目のビルドの結果がまだ出ていない。これが成功したら
`.nojekyll` は不要になる。
