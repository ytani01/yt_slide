...
その後、利用者から「公開手順については、詳しく説明する必要はない」と指定された。
上記 1 の詳しい手順の追加は見送り、公開については必要なファイルと共有 URL の
短い案内にとどめる。公開先の用意やサービス別の操作説明は追加しない。
...
- [ ] 公開については必要なファイルと共有 URL を短く案内し、既存の
  `docs/User.md` の「公開」へリンクする。
...
- [ ] reviewer が README・ガイド・対応するスライドと実装を照合し、上記 2〜5 点への回答が
  見つかること、公開の案内が指定された範囲に収まること、説明の食い違いが無いことを確認する。
- [ ] reviewer の指摘を反映した後、verifier が空の作業ディレクトリから掲載手順を再現する。
  最小の例を保存し、ブラウザで `player.html?slides=sample` の本文と読み上げを確認する。
- [ ] 公開に必要なファイルと共有 URL の案内を、既存の `docs/User.md` の「公開」と照合する。
