...
**残っている項目: TODO-107。** これまでに 106 件を決着させた。
新しく足すときは「完了済み」の上に節を作る。**番号は `TODO-108` から。**
...
- [ ] 見本を指している「型」を「テンプレート」に置き換える。対象は
      `grep -rn "型" README.md docs/*.md slides/*.js index.html` で拾う
      （62 箇所。ただし「縦型・横型」「定型」など別の意味のものが混じる）
- [ ] narration を変えた枚の `duration` を測り直す
- [ ] verifier に、置き換え漏れと誤った置き換えの有無、測り直した
      `duration`、書き換えた枚の描画を確かめさせる

「型」が伝わりにくい。ファイル名が `slides/template.js`、URL も
`?slides=template` なので、呼び方だけ「型」なのは一段ずれている。
...
`slides/template.js` 自身の本文（27 箇所）には、見本としての「型」以外に
別の意味のものが混じる。機械的に置き換えず、1 件ずつ見る。
