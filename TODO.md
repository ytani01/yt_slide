...
**残っている項目: TODO-111。** これまでに 110 件を決着させた。
新しく足すときは「完了済み」の上に節を作る。**番号は `TODO-112` から。**
...
- [ ] 「枚」を名詞として使っている箇所を「スライド」に直す
- [ ] 文法が壊れている箇所を直す
- [ ] 紛らわしい語を言い換える
- [ ] 表記ゆれを揃える

TODO-110 で `docs/User.md` の「直す枚」を直したあと、同種のものが他にも
無いか調べた。挙動は変えず、文言だけを直す。

### 1. 「枚」を数詞ではなく名詞として使っている（TODO-110 と同じ形）

`1 枚`・`3 枚目`・`枚数` のように数詞として使っている箇所は直さない。

- `docs/User.md:503` 見出し「「この枚を見て」と渡す」
- `docs/User.md:512` 「その枚から始まる」「同じ枚に戻る」
- `docs/User.md:482` 「その枚だけ書き出す」
- `docs/User.md:174` 「全枚の読み上げ秒数」
- `docs/Developer.md:151` 「その枚から始める」
- `player.html:1163` `// 見ている枚を URL に載せる`
- `slides/template.js:6` 「他の枚には影響しない」

### 2. 文法が壊れている・意味が取りにくい

- `docs/Developer.md:100-102` 「17 枚は `ffprobe` で測定し、入れた値
  （合計 324 秒）で、目分量ではない。」— 述語がつながっていない
- `docs/Developer.md:114` 「同じ値なため」、`:266` 「そのものなため」—
  名詞に「な + ため」が付いていて不自然（`:264` の「十分なため」は
  形容動詞なのでこのまま）
- `docs/Developer.md:35` 「（オフラインでは崩れる）。`public_html/` に
  置くため、ファイルを置くだけで公開される。」— 前文と関係の無い話が
  同じ段落に続き、「置く」も重複している

### 3. 語の選び方が紛らわしい

- `src/ytslide/measure.py:5` 「当たるスライドを測り直す」— ここの「当たる」は
  「該当する」の意味だが、同じファイルの `:9` や `docs/User.md:440` の
  「当たる」「当てる」は置換規則が適用される意味。`docs/Developer.md:105` の
  「対象スライド」に揃える
- `player.html:970` 「落ち着いた優艶な日本語女性音声」— 「優艶」は普通
  使わない言い方
- `player.html:1459` 「読み上げの解錠（unlockFallbackAudio）」—
  `docs/Developer.md:220` は「unlock している」と書いている

### 4. 表記ゆれ

- `docs/Developer.md:103` 「読み置換表」— 他は「読みの置換表」
  （`src/ytslide/measure.py:7`、`docs/User.md`）
- 数字と日本語の間の半角スペース。文書は「1 つ」「19 種」と空け、
  スライドの表示されるテキストは「1要素が1枚」「読み上げの2系統」と
  詰めていて揃っていない。**詰める方に統一する**（利用者の判断）。
  - 詰めるのは**数を数える表現だけ**。`1 つ`・`19 種類`・`3 枚目`・
    `2 カラム`・`1 回` などの助数詞が対象
  - **比率・バージョン・測定値は空けたまま。** `16:9 の外枠`、
    `Python 3.13 以上`、`0.75〜2.0 倍`、`324 秒`、`180 字`
  - 対象は `README.md`・`AGENTS.md`・`CLAUDE.md`・`docs/`・`index.html`・
    `player.html`・`slides/`・`src/`・`tests/` の約 126 箇所（19 ファイル）。
    コメントと docstring も含める
  - **`TODO.md` の完了済みの目次と `archives/` は触らない。** 目次の見出しは
    `archives/todo/` のファイル名と一致させてあり、`archives/` は
    そのときの記録のため
- `slides/user.js:6` の `title` 「player.html で別のスライドを作る」—
  README・`docs/User.md` は「自分のスライドを作る」。「別の」は何と別なのかが
  分からない
- `slides/user.js:58` の `narration` 「〜をご紹介します」— 他のナレーションは
  「〜します」で、ここだけ丁寧度が高い

### 確認

`slides/*.js` の `title` と本文は画面に出るので、直したあとブラウザで
表示を見る担当を分ける。`player.html`・`.py` はコメントと docstring だけを
変えるので、`uv run pytest` が通ることと、`ytslide measure --help` などの
表示が壊れていないことを見る。
...
- [**TODO-109.** player のヘッダー左のアイコンからスライド一覧へ戻れるようにする](archives/todo/TODO-109.%20player%20のヘッダー左のアイコンからスライド一覧へ戻れるようにする.md)
- [**TODO-108.** player のヘッダー左をアイコン・タイトル・JS ファイル名だけにする](archives/todo/TODO-108.%20player%20のヘッダー左をアイコン・タイトル・JS%20ファイル名だけにする.md)
- [**TODO-107.** スライドの見本の呼び方を「型」から「テンプレート」に変える](archives/todo/TODO-107.%20スライドの見本の呼び方を「型」から「テンプレート」に変える.md)
- [**TODO-106.** 冒頭の謳い文句を「1 から書かなくていい」に変え、AI で作る手順を書く](archives/todo/TODO-106.%20冒頭の謳い文句を「1%20から書かなくていい」に変え、AI%20で作る手順を書く.md)
- [**TODO-105.** README からスライド作成・公開までの流れを分かるようにする](archives/todo/TODO-105.%20README%20からスライド作成・公開までの流れを分かるようにする.md)
- [**TODO-104.** 本文の既定サイズを枠に追従させる](archives/todo/TODO-104.%20本文の既定サイズを枠に追従させる.md)
