...
**残っている項目: TODO-103。** これまでに 102 件を決着させた。
新しく足すときは「完了済み」の上に節を作る。**番号は `TODO-104` から。**
...
- [ ] 「自分のスライドを作る」を「型を選んでコピーし、題名・読み上げる
      文章・本文を差し替える」に書き換える。`template.js` の 19 種に触れる
- [ ] AI に作らせる案内を 1 行足す（`template.js` と `docs/User.md` を渡す）
- [ ] 「すぐ試す」の先頭にデモ URL（https://ytani01.github.io/yt_slide/）
- [ ] 末尾にライセンス（MIT）の 1 行

`docs/User.md`

- [ ] 「最小の例」を `body: '<p>本文はここに書く</p>'` に差し替え、
      `clamp` と `cqw` は「文字の大きさを変えたくなったら」の節へ送る
- [ ] 手順 2 に「`template.js` から近い型をコピーする」道を併記する
- [ ] AI に作らせる案内を手順の中にも書く

確認（verifier）

- [ ] 装飾なしの `body` を `player.html` で実際に開き、読めるか・崩れて
      困るかを見る。困るなら最小の例に何を足すか報告させる
- [ ] デモ URL が実際に開くか
- [ ] README と `docs/User.md` の間で食い違いが出ていないか

（背景）

README を読んだスライド作成者が不安になる箇所を洗った。中身は
`{title, icon, duration, narration, body}` の並びで JavaScript の文法は
ほぼ要らないのに、README の「本文の HTML を書けば」と、`docs/User.md` の
「最小の例」に出てくる `clamp(1.15rem, 2.8cqw, 2.1rem)` で、書けない人が
そこで降りる。実際の作り方は `template.js` からのコピーと差し替えなのに、
それが `docs/User.md` の中ほどにしか書いていない。

デモ URL は TODO-087 で公開済み（https://ytani01.github.io/yt_slide/）だが、
README から一度も指していない。ライセンスは `LICENSE`（MIT）があるが
README に記載が無い。

（今回は入れないもの）

- **壊れたとき（画面が真っ白）の逃げ道。** カンマ・バッククォート・閉じ
  タグの抜けと、コンソールの見方。選択肢に出したが選ばれなかった
- **TTS が外部サービスに依存する件の説明**（聞く人の環境から Google へ
  ナレーション文が送られる、止まったらどうなるか）。別項目に回す
- **README から `docs/User.md` の「公開」への直リンク。** 別項目に回す
- **`slides/readme.js`・`user.js` への反映。** 別項目に回す
