...
**残っている項目: TODO-088、TODO-089、TODO-090。** これまでに 87 件を決着させた。
...
- [ ] `player.html` と `slides/readme.js` だけを別のディレクトリに置いて動くか実測する
- [ ] `file://` で音声（Online Voice / Web Speech API）が鳴るか実測する
- [ ] 実測を踏まえ、文言の案を利用者と決める
- [ ] 決まった文言に直す
- [ ] 直した文言でスライドが読まれることを確認する

`README.md` と `slides/readme.js` の冒頭が並べている主張のうち、実態と
ずれているものがある。きっかけは「URL を渡せばそのまま見てもらえる」への
指摘で、そこだけ直すつもりだったが、周辺の主張もまとめて見たほうがよい。

見直す主張は 4 つ。

| 主張 | 現状の見立て |
|------|--------------|
| ファイル 2 つを置くだけで動く | 通りそう。`player.html` + `slides/<名前>.js` で、`readme.js` は `images/` を参照していない。ただし未実測 |
| ビルドもインストールも不要 | 通る |
| URL を渡せばそのまま見てもらえる | **配信の手間を飛ばしている。** URL がある状態＝すでにどこかに配信している状態 |
| すぐ試すには簡易サーバーを立てる | **描画だけなら要らない。** `file://` で `index.html` も `player.html?slides=readme` も、コンソールエラー 0 件・本文 432 文字で動いた（TODO-087 で実測）。音声は未確認 |

後ろ 2 つは、ずれの向きが反対になっている。片方は手間を少なく言い、
もう片方は多く言っている。

対象箇所（`archives/` を除く）:

```
README.md:3     ファイル 2 つを置くだけで、ナレーション付きのプレゼンが動く。
README.md:6     動画に書き出す必要はなく、URL を渡せばそのまま見てもらえる。
README.md:10    ビルドもインストールも不要。… Web サーバーに置くだけで公開できる。
README.md:25    python3 -m http.server 8000
slides/readme.js:5    title    ファイル 2 つで動くナレーション付きプレゼン
slides/readme.js:23   narration  2 つのファイルを置くだけで … URLを渡すだけでそのまま見てもらえます
slides/readme.js:35   タイトルスライド本文  URL を渡せばそのまま見てもらえる。
slides/readme.js:82   narration  Pythonの簡易サーバーを立てて player.html を開くだけです
slides/readme.js:265  narration  ファイル 2 つを置くだけで … すぐに作って公開できます
slides/readme.js:275  まとめ  ファイル 2 つ を置くだけで、ナレーション付きプレゼンが動く
slides/readme.js:283  まとめ  ビルド不要。URL を渡すだけでそのまま見てもらえる
```

洗い出しに使ったコマンド:

```bash
grep -rnE 'URL ?を渡|そのまま見て|ファイル ?2 ?つ|2 ?つのファイル|サーバー' \
  --include='*.js' --include='*.md' --include='*.html' . | grep -v '^./archives/'
```

決めること: **「サーバーが要る」と書くかどうか。** 音声が `file://` で
鳴るなら `readme.js:82` の簡易サーバーの説明自体が要らなくなり、鳴らない
なら「描画は `file://` で足りるが、音声にはサーバーが要る」と書き分ける
ことになる。**実測の前には案を出せない**ので、実測のチェックが先。

`README.md` は :6 の直後、:10 で「Web サーバーに置くだけで公開できる」と
補っている。単体ではそこまで誤解を招かないので、直す重みはスライド側より
軽い。ただし主張を揃えるなら一緒に見る。

ナレーションを変えるので、**文言が利用者と固まるまで確認の担当は起こさない。**
`wording` は明示の依頼が無い限り使わない。

---

## TODO-089. `player.html` の中でスライドの一覧を自動で取得する方法を検討する

|        | main                 | 担当     |
|--------|----------------------|----------|
| 見込み | Opus 5 / effort high | verifier |

- [ ] `file://`・Apache・GitHub Pages の 3 つで、`slides/` の中身が
      JavaScript から列挙できるか実測する
- [ ] 環境によって取れないときの落としどころを利用者と決める
- [ ] 決めた方式を実装するかどうかを決める（実装は別項目にしてよい）

いまのスライドの一覧（`index.html`）は `<li>` を手で書いたリンク集で、
`slides/` を見て作られるわけではない。そのため、

- スライドを足したら `index.html` にも 1 つ足す（`docs/User.md` の「手順」3）
- 他のサーバーへ持っていくときは、持っていかないスライドの `<li>` を消す
  （TODO-088 で `docs/User.md` に明記した）

という手作業が要る。これを `player.html` の側で `slides/` から自動で
集められないか、という話。

**取れるかどうかは配信環境しだい**で、そこが検討の中心になる。

| 環境 | 見立て |
|------|--------|
| `file://` | ディレクトリの列挙は `fetch` がブロックされて取れないはず。未実測 |
| Apache など autoindex のあるサーバー | ディレクトリ一覧の HTML を取って解析すれば取れるはず。ただし出力形式はサーバーの設定しだい。未実測 |
| GitHub Pages | autoindex が無いので取れないはず。未実測 |

TODO-088 で「`file://` でも表示から読み上げまで動く」と書いたばかりなので、
**`file://` で動かない方式は採りにくい**。

決めること: **どこまで自動にするか。** 全環境で動く方式が無い場合、
次のどれを採るか（実測の前には案を出せないので、実測のチェックが先）。

- 取れる環境でだけ自動、取れない環境では今の `index.html` にそのまま戻す
- 一覧を書いたファイル（`slides/_list.js` のような）を 1 つ置き、
  `index.html` と `player.html` の両方がそれを読む。手で書く点は変わらないが、
  書く場所が 1 つになる
- 自動化はせず、今のまま（**対応しない**）

**ビルドで一覧を生成する案は採らない。**「ビルドもインストールも不要」を
`README.md` の冒頭に掲げているため。
...
- [ ] 目分量の `duration` で実際に何が起きるか実測する
- [ ] 実測を踏まえ、`docs/User.md` の書き方を利用者と決める
- [ ] 決まった文言に直す

`README.md` の冒頭は「ビルドもインストールも不要」と掲げているが、
`duration` を合わせるには `tools/measure-duration.py` を走らせることになり、
それには `curl` と `ffprobe` が要る。ビルドとは違うが、**スライドを作る側には
道具が要る**という点で、冒頭の主張と擦れる。

`docs/User.md:78` は `duration` を「**読み上げの実測値を入れる**」と書いていて、
**目分量の数字でもよいことが読み取れない**。実際にどこまで困るのかが分かれば、
書き方を弱められる。

コードを読んだ範囲では次のようになっている（**未実測**）。

| 場面 | `duration` の役割 |
|------|-------------------|
| 音声あり | 進行バーと残り時間の表示。送りは読み上げの終了で起きる（`player.html:1068` は `onended` が来ないときの受け皿） |
| 消音中 | `player.html:917` で `duration / playbackRate` がそのまま送りの間隔になる |

実測すること:

- 目分量の `duration`（実測値から大きくずらした値）で、音声ありの送りが
  ずれないこと、進行バーがどうなるか
- 消音中の送りの間隔が `duration` どおりになること
- `duration` を**書かなかった**ときに何が起きるか（`player.html:657` の
  `slideData[index].duration + pauseSeconds` が `NaN` になるはず）

決めること: **`docs/User.md` で「目分量でもよい」とどこまで言うか。**
実測の前には案を出せないので、実測のチェックが先。

`README.md` の冒頭に「見る・公開するなら」という限定を付けるかは、
**TODO-088 で付けないと決めた**（冒頭の主張は見る側・公開する側の話で、
`README.md:37-39` が「前提が要るのは `tools/` のスクリプトを使うときだけ」と
既に書き分けている）。
