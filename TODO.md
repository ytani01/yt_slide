...
**残っている項目: TODO-110。** これまでに 109 件を決着させた。
新しく足すときは「完了済み」の上に節を作る。**番号は `TODO-111` から。**

---

## TODO-110. docs/User.md の「直す枚」を「直すスライド」に直す

- [ ] `docs/User.md:127` の「直す枚をスライド番号で指し」を
      「直すスライドを番号で指し」に直す
...
「枚」を数詞ではなく名詞として使っていて通じない。文書全体が
「3 枚のスライド」「3 枚目」と数えているので、「ページ」にすると
数え方が混ざる。「スライド」に戻すと「スライド番号」の重複も消える。

文書 1 行だけの変更で、確かめる中身が書式の揃いしか無いため、
確認は main が行う（`~/.claude/CLAUDE.md` の例外）。
