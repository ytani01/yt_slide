...
**残っている項目: TODO-069。**
これまでに 68 件を決着させた。
新しく足すときは「完了済み」の上に節を作る。**番号は `TODO-070` から。**
...
- [ ] 用途別に 20〜30 件のアイコンを選ぶ
- [ ] `docs/User.md` の「最小の例」の後に付録の節を足す
- [ ] `slides/user.js` の「まとめ」の後にスライドを足す（用途別に分ける）
- [ ] 足したスライドの `duration` を `tools/measure-duration.py` で測って書き戻す
- [ ] verifier に表示と 2 箇所の食い違いを確認させる

`icon` に何を書けるかの手がかりが、今は `fa-list-check` の 1 例しか無い。
FontAwesome のサイトを見に行かないと選べないので、よく使う型（箇条書き、
注意、手順、コード、時間、人、図表、矢印）ごとに 3〜5 件ずつ挙げておく。

- 対象は FontAwesome 6.5.1 の無料分（`player.html` が CDN から読む版）。
  **リポジトリで未使用のものも含める**ので、実際に描画されるかの確認が要る
  （名前を間違えると何も出ない）
- スライドは読みやすさを優先し、用途別に何枚に分けてもよい
- 同じ一覧が User.md とスライドの 2 箇所に載る。食い違わないよう突き合わせる
- 挙動は変わらない（スライドのデータを足すだけ）ので reviewer は立てない
