...
**残っている項目: TODO-091・TODO-092。** これまでに 90 件を決着させた。
...
- [ ] Web Speech API の `getVoices()` から日本語の声を拾い、プルダウンに出す
- [ ] 選んだ声を `saveSetting` で覚え、次に開いたときに復元する
- [ ] 覚えた声が無い端末では既定（今の自動選択）に戻す
- [ ] `docs/Developer.md` の読み上げの節を実装に合わせる

いまは Online Voice（`translate_tts`）と Web Speech API の 2 択で、
声そのものは選べない。しかも Web Speech の音声選択（`player.html` の
`femaleJaVoice`）は名前に `google` を含むものを優先するため、Chrome では
どちらを選んでも Google の同じ日本語音声になり、「切り替えても声が
変わらない」と見える。
...
- **端末の音声一覧だけを対象にする。** VOICEVOX（tts.quest）などの外部
  サービスと、音声ファイルの事前生成は今回やらない
- Online Voice は残す。日本語の声が 1 つも無い端末があるため
- **UI はプルダウンメニュー（`<select>`）にする。** 今の 2 択のトグル
  ボタンは、その 1 つ目の項目「Online Voice」に畳んで置き換える。
  2 つ目以降が端末の日本語音声で、選ぶとその声で Web Speech を使う

確認について:

- **ヘッドレスの Chromium には音声合成エンジンが無く、Web Speech は
  自動では鳴らせない**（TODO-088 で実測）。verifier には一覧の組み立て・
  保存・復元・既定への戻りまでを見てもらい、実際の声の違いは利用者が
  実機で聞いて確かめる
...
- [ ] 声のプルダウンの左にあるスピーカーマークを、消音の切り替えボタンにする
- [ ] 操作パネルの `mute-btn` を外す
- [ ] マークを消音中とそうでないときで見分けられるようにする
- [ ] `docs/User.md`・`docs/Developer.md`・操作ガイドの記述を合わせる

いまスピーカーマークは飾りで、消音は再生ボタンの列にある `mute-btn` で
切り替える。音に関する操作をヘッダーの 1 か所にまとめる。

決めたこと（背景。項目にはしない）:

- **消音ボタンは 1 か所にする。** 操作パネルの `mute-btn` は外し、
  ヘッダーのマークに移す（2 か所に置くと状態表示を両方で揃えることになる）
- `M` キーでの切り替えはそのまま残す

**TODO-091 が決着してから着手する**（同じヘッダーの同じ場所を触るため）。
