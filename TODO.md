...
**残っている項目: TODO-071。**
これまでに 70 件を決着させた。
新しく足すときは「完了済み」の上に節を作る。**番号は `TODO-072` から。**

---

## TODO-071. `curl` のループをバックグラウンドで走らせると固着する件を切り分ける

|      | main | 担当 |
|------|------|------|
| 見込み | Opus 5 / effort medium | main のみ |

- [ ] 再現するか確かめる（`while read` + `curl` をバックグラウンドで）
- [ ] 固着する条件を絞る（バックグラウンドか、ループか、`curl` か、通信先か）
- [ ] 分かったことの置き場所を決める（このリポジトリか `~/.claude/CLAUDE.md` か）

TODO-070 で verifier が踏んだ。`curl` を `while read` ループの中で使う
スクリプトをバックグラウンドで実行したところ、**プロセスが応答なく固着し、
10 分以上 1 行も出力されなかった**。強制終了して `urllib.request` の
フォアグラウンド実行に切り替え、目的は達成している。
[verifier の報告](archives/agents/TODO-070/verifier-report.md)。

- **原因は切り分けていない。** verifier は「ネットワークかシェルの
  クォート展開かは不明」と報告している
- TODO-070 の結果への実害は無い（別の手段で 30 件とも取得できている）
- `~/.claude/CLAUDE.md` には、`cp`/`mv`/`rm` のエイリアスで確認プロンプトが
  出て固まる件と、`pkill` が自分のシェルを巻き込む件が既にある。
  **同じ「シェルで固まる」系なら、そちらに並べて書く**のが筋
- **決めること**: 切り分けた結果、どこまで面倒を見るか。
  「使わない手順を書いて終わり」か、「再現の条件まで詰める」か。
  **原因が分かってから聞く**（今は選択肢を出せない）
