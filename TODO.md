...
**残っている項目: TODO-072。** これまでに 71 件を決着させた。
新しく足すときは「完了済み」の上に節を作る。**番号は `TODO-073` から。**
...
- [ ] `~/.claude/CLAUDE.md` の「サブエージェントへの依頼」に 1 項目足す

TODO-066（`make-video.py`）で、確認の依頼文に「動画の画質・デザインの
良し悪しは見なくてよい」と書いた。その結果、verifier は抜き出したフレームを
見ていながら、**フェードの途中で撮っている不具合**を「フェードイン演出」として
流した。気づいたのは main が同じフレームを自分で見たとき。担当の落ち度ではなく、
依頼の切り方の問題。

**「デザインの良し悪しを評価しない」と「映っているものが正しいか見る」は別。**
現行の `~/.claude/CLAUDE.md` は「見なくてよいものを、依頼で名指しで外す」を
節の先頭に置いていて、外す側だけが書いてある。映像や画像を作る項目では、
フレームを抜いて**不透明か・欠けていないか・余計なものが映っていないか**を
見るところまでを確認の担当の仕事にする、という線引きを足す。

文言は着手時に決める。書き換えるのはリポジトリ外の `~/.claude/CLAUDE.md` の
1 箇所だけなので、担当は `main のみ`（確かめる中身が書式しか無い場合の例外）。

TODO-066 の振り返りにはこれと並んで「`ffprobe` の数値が揃っていることを
『動いた』と読まない」もある。同じ節に足すかどうかは着手時に決める。
