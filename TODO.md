...
**残っている項目: TODO-074・TODO-075・TODO-077・TODO-078・TODO-083。** これまでに 78 件を決着させた。
...
- [ ] 「ポイント・注意の帯」の型を足す（強調と注意の 2 色）
- [ ] 「手順（横並び）」の型を足す（番号付きのカードを左から右へ並べる）
- [ ] 「カード 3 枚」を「カード（段数の違う行を混ぜる）」に差し替える
- [ ] 枚数の記述を 19 に直す（先頭のコメント・`slidesConfig.heading`・表紙・
      7 枚目の数字・13 枚目の Q&A）
- [ ] `docs/User.md` と `README.md` の型の一覧を直す
- [ ] `tools/measure-duration.py` で `duration` を測り直す
...
- **「ポイント・注意の帯」**。横幅いっぱいの 1 行に、アイコンと短い文を入れる箱。
  `claude-memo.js` では 4 枚で使っている（3 枚目の「ポイント:」、6 枚目の
  「※ 担当とモデルは固定ではない」、10 枚目の赤い断り書き、17 枚目の締め）。
  強調（緑系）と注意（赤系）の 2 色を見せる。いまのテンプレートには
  「ここだけ強く言う」書き方が無い
- **「手順（横並び）」**。番号付きのカードを左から右へ並べる
  （`claude-memo.js` 4 枚目「要望 → 起草 → 完走 → 保存」）。既存の「時系列」は
  縦並びなので別の型にする
- **「カード 3 枚」は「カード（段数の違う行を混ぜる）」に差し替える。**
  全幅 1 行、次に 2 列、最後に 3 列の順に並べ、1 枚の中で段数を変える書き方
  （`col-span` の使い方）を見本に残す。「カード 3 枚」自体はこの型に含まれる
- 目次の型は立てない（この型で足りる）
- `claude-memo.js` の 3 枚目・7 枚目・11 枚目・12 枚目・13 枚目のパターンは、
  既存の型（カード・コードと端末画面・表・本文と脚注）で足りるので足さない

型は 17 種から 19 種になる。`slideData` の配列を足し引きするだけで挙動は
変わらない。verifier が Playwright で 1280×720 と 390×844 を確認する。
全 19 枚が描画エラー無しで出る、足した 2 種と差し替えた 1 種が枠から
はみ出さない、枚数の記述に書き残しが無い、`docs/User.md` の型の一覧が
実物と合っていることを見る。
