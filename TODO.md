...
**残っている項目: TODO-085。** これまでに 84 件を決着させた。
新しく足すときは「完了済み」の上に節を作る。**番号は `TODO-086` から。**

---

## TODO-085. リポジトリに LICENSE を置く

- [ ] MIT ライセンス全文の `LICENSE` をリポジトリのトップに作る

|      | main | 担当 |
|------|------|------|
| 見込み | Opus 5 / effort high | main のみ |

このリポジトリにはライセンスの表示が無い。公開しているものなので、
利用条件を明示する。

- ライセンスは **MIT**。他の自作リポジトリ（tmr, ytstreetorgan）と揃える
- 著作権表記は `Copyright (c) 2026 Yoichi Tanibayashi`
- **`LICENSE` 1 枚だけ。** `README.md` や `docs/`、スライドには手を入れない
- 定型の全文を置くだけで、確かめるのは文面が MIT 標準と一致するかだけ
  なので、担当は `main のみ`
