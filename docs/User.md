...
まず自分の内容を 1 枚作って再生する。その後は、編集して確認する作業を繰り返す。
...
`template.js` はテンプレートで、最初の一覧には `template` が 1 件載る
（開くと 19 種類が並ぶ）。
`init` は既存のファイルをコピーで上書きせず、最後に一覧を再生成する。
新しく置く `index.html` のタイトルと見出しにはディレクトリ名が入る。
`README.md` は置かないが、自分で同じ場所に置けば一覧ページに表示される。
...
**`slides/template.js` からコピーして始めてもよい。** 19 種類の
テンプレートが入っているので、`player.html?slides=template` で見て
近いものを選び、その 1 枚をコピーして中身を差し替えるほうが、
1 から書くより早い（手順は次の 3.）。
...
開いて選ぶ。エディターで `slides/template.js` を開き、使いたい 1 枚の
...
- **作業場所の `slides/template.js`** — テンプレート 19 種。渡すと、
  近いものに沿った見た目で返ってくる
...
Online TTS で全枚の読み上げ秒数を測り、`sample.js` の `duration` を書き換えてから
...
19 種類。使いたいテンプレートのスライドをまるごとコピーして、
