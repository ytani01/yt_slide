...
`template.js` は型の見本で、最初の一覧には `template` が 1 件載る
（開くと 19 種類が並ぶ）。
...
**`slides/template.js` からコピーして始めてもよい。** 19 種類の型が
...
型をコピーするときは、同じプレイヤーを `?slides=template` で開いて型を選ぶ。
エディターで `slides/template.js` を開き、使いたい 1 枚のオブジェクト
（`{` から対応する `},` まで）を `sample.js` の `slideData` の中へコピーする。
