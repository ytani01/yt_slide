...
- [template の縦型対応案のレビュー](template-review.md): 全19種類のコードを根拠に、
  配置変更の懸念と対応案を TODO.md に追記した内容を確認する。
  今回は配置の意味や内容保全を判断するため、main の調査・文書作成とは別に
  reviewer（gpt-5.6-sol / high）を割り当てた。ブラウザ実測は今後の実装時に行う。
